...
# output from each test i is stored in test2-$i.log and test2-$i.err (STDOUT and
# STDERR respectively).
#
...
logs=$(find . -maxdepth 1 -name 'test2-*.log' -type f -printf '.' | wc -c)
success=$(grep -E '^PASS$' test2-*.log | wc -l)
((failed = logs - success))
...
cleanup() {
	for pid in "${waits[@]}"; do
		kill "$pid"
		wait "$pid"
		rm -rf "test2-${is[0]}.err" "test2-${is[0]}.log"
		is=("${is[@]:1}")
	done
	exit 0
}
...
for i in $(seq "$((success+failed+1))" "$runs"); do
	...
	if [[ -z "$test" ]]; then
		./tester2 -test.v 2> "test2-${i}.err" > "test2-${i}.log" &
		pid=$!
	else
		./tester2 -test.run "$test" -test.v 2> "test2-${i}.err" > "test2-${i}.log" &
		pid=$!
	fi
